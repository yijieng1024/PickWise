...
const require_standard = require('../utils/standard.cjs');
...
var ChatOpenAIResponses = class extends require_base.BaseChatOpenAI {
	...
	_convertResponsesMessageToBaseMessage(response) {
		...
		return new __langchain_core_messages.AIMessage({
			...
			tool_calls,
			invalid_tool_calls,
			usage_metadata: require_output._convertOpenAIResponsesUsageToLangChainUsage(response.usage),
			additional_kwargs,
			response_metadata
		});
	}
	...
	_convertResponsesDeltaToBaseMessageChunk(chunk) {
		...
		if (chunk.type === "response.output_text.delta") content.push({
			...
			usage_metadata = require_output._convertOpenAIResponsesUsageToLangChainUsage(chunk.response.usage);
	...
	_convertMessagesToResponsesParams(messages) {
		return messages.flatMap((lcMsg) => {
			...
			if (responseMetadata?.output_version === "v1") return require_standard._convertToResponsesMessageFromV1(lcMsg);
			const additional_kwargs = lcMsg.additional_kwargs;
			let role = require_misc.messageToOpenAIRole(lcMsg);
			if (role === "system" && require_misc.isReasoningModel(this.model)) role = "developer";
			...
			if (role === "assistant") {
				...
				if ((0, __langchain_core_messages.isAIMessage)(lcMsg) && !!lcMsg.tool_calls?.length) input.push(...lcMsg.tool_calls.map((toolCall) => {
			...
			if (role === "user" || role === "system" || role === "developer") {
				...
				const content = lcMsg.content.flatMap((item) => {
					...
					if ((0, __langchain_core_messages.isDataContentBlock)(item)) return (0, __langchain_core_messages.convertToProviderContentBlock)(item, require_message_inputs.completionsApiContentBlockConverter);
					if (item.type === "text") return {
						type: "input_text",
						text: item.text
					};
