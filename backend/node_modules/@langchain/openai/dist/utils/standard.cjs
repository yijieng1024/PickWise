...
function _convertToChatCompletionsData(block) {
	...
	if (block.type === "audio") {
		if (block.data) {
			const format = require_misc.iife(() => {
...
function _convertToResponsesMessageFromV1(message) {
	const isResponsesMessage = (0, __langchain_core_messages.isAIMessage)(message) && message.response_metadata?.model_provider === "openai";
...
//#endregion
exports._convertToCompletionsMessageFromV1 = _convertToCompletionsMessageFromV1;
exports._convertToResponsesMessageFromV1 = _convertToResponsesMessageFromV1;
